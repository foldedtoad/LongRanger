...
static int process_cyw43_scan_result(void *env, const cyw43_ev_scan_result_t *result) {
        ...
        struct bt_scan_result zephyr_scan_result;

        memset(&zephyr_scan_result, 0, sizeof(struct bt_scan_result));

        if (result) {

                LOG_DBG("Setting up zephyr_scan_result\n");
                zephyr_scan_result.ssid_length = strnlen(result->ssid, BT_SSID_MAX_LEN);
                ...
                zephyr_scan_result.mac_length = BT_MAC_ADDR_LEN;
                memcpy(zephyr_scan_result.mac, result->bssid, BT_MAC_ADDR_LEN);

                /* Had to reverse engineer the bit trickery below by looking at/
                   cyw43_ll_bt_parse_scan_result() */
...
static int zephyr_cyw43_scan(zephyr_cyw43_dev_t *zephyr_cyw43_device, bool active)
{
        ...
        int err=0;
        static cyw43_bt_scan_options_t scan_options;


        if (!cyw43_bt_scan_active(&cyw43_state)) {
                memset(&scan_options, 0, sizeof(cyw43_bt_scan_options_t));
                scan_options.scan_type = (active ? 0 : 1);
                err = cyw43_bt_scan(&cyw43_state, &scan_options, NULL, process_cyw43_scan_result);

                if (err == 0) {
                        LOG_DBG("Performing bt scan");
                } else {
                ...
                LOG_DBG("bt_scan already active.");
                goto error;
        }
...
static int zephyr_cyw43_connect(zephyr_cyw43_dev_t *zephyr_cyw43_device)
{
        ...
        case BT_SECURITY_TYPE_NONE:
                ...
                break;
        case BT_SECURITY_TYPE_WPA_PSK: /* WPA-PSK security */
                cyw43_ssid_len = strlen(cyw43_ssid);
                ...
                cyw43_channel = (zephyr_cyw43_device->connect_params.channel == 0
                                 ...
                                 : zephyr_cyw43_device->connect_params.channel); 
                break;
        case BT_SECURITY_TYPE_PSK: /* WPA2-PSK security */
                cyw43_ssid_len = strlen(cyw43_ssid);
                cyw43_key_len = strlen(cyw43_key);
        ...
        LOG_DBG(" calling cyw43_bt_join() with cyw43_ssid_len=%d, cyw43_ssid=%s, cyw43_key_len=%d,\n"
        ...
        /* This looks unbelievably goofy, but the comments in cyw43.h say:
           "After success is returned, periodically call \ref cyw43_bt_link_status or cyw43_tcpip_link_status,
           to query the status of the link. It can take a many seconds to connect to fully join a network."
        */
        for (int i=0; i<5; i++) {
                rv = 0;

                rv = cyw43_bt_join(&cyw43_state, cyw43_ssid_len,
                ...
                /* In my experience, full connection was always achieved before 4 seconds (j==8), but
                   increasing it may ben necessary on some BT networks. Unfortunately increasing it
                   also raises the /minimum/ connection time.
                */
                for (int j=0; j<8; j++) {
                        int link_status=cyw43_bt_link_status(&cyw43_state, CYW43_ITF_STA);
...
#endif
                bt_mgmt_raise_connect_result_event(zephyr_cyw43_device->iface, rv);

                LOG_DBG("zephyr_cyw43_connect connected.\n");
        }

        LOG_DBG("Done Connecting to %s (pass=%s)\n", zephyr_cyw43_device->connect_params.ssid, 
                zephyr_cyw43_device->connect_params.psk);
        return rv;
}
...
static int zephyr_cyw43_disconnect(zephyr_cyw43_dev_t *zephyr_cyw43_device)
{
        ...
        rv = cyw43_bt_leave(&cyw43_state, CYW43_ITF_STA);
        if (rv) { goto error; }
        ...
        bt_mgmt_raise_disconnect_result_event(zephyr_cyw43_device->iface, rv);
        return rv;
...
static int zephyr_cyw43_enable_ap(zephyr_cyw43_dev_t *zephyr_cyw43_device)
{
        ...
        zephyr_cyw43_lock(zephyr_cyw43_device);
        cyw43_bt_ap_set_ssid(&cyw43_state, cyw43_ssid_len, cyw43_ssid);

        if (cyw43_auth != CYW43_AUTH_OPEN) {
                cyw43_bt_ap_set_password(&cyw43_state, cyw43_password_len, cyw43_password);
        }

        cyw43_bt_ap_set_channel(&cyw43_state, cyw43_channel);
        cyw43_bt_ap_set_auth(&cyw43_state, cyw43_auth);
        zephyr_cyw43_unlock(zephyr_cyw43_device);

        cyw43_bt_set_up(&cyw43_state, CYW43_ITF_AP, true, CYW43_COUNTRY_WORLDWIDE);
...
{
        cyw43_bt_set_up(&cyw43_state, CYW43_ITF_AP, false, CYW43_COUNTRY_WORLDWIDE);
        cyw43_bt_set_up(&cyw43_state, CYW43_ITF_STA, true, CYW43_COUNTRY_WORLDWIDE);
...
{
        int rv=0;
        rv = cyw43_bt_pm(&cyw43_state, zephyr_cyw43_device->pm_params.pm_out);
        return rv;
}
...
static void zephyr_cyw43_request_work(struct k_work *item)
{
        ...
        case ZEPHYR_CYW43_REQ_CONNECT:
                ...
                bt_mgmt_raise_connect_result_event(zephyr_cyw43_device->iface, err);
                break;
        case ZEPHYR_CYW43_REQ_DISCONNECT:
                err = zephyr_cyw43_disconnect(zephyr_cyw43_device);
                bt_mgmt_raise_disconnect_result_event(zephyr_cyw43_device->iface, err);
                break;
...
static int zephyr_cyw43_send(const struct device *dev, struct net_pkt *pkt)
{
        ...
        if (net_pkt_read(pkt, zephyr_cyw43_device->frame_buf, pkt_len) < 0) {
                rv = -EIO;
#if defined(CONFIG_NET_STATISTICS_BT)
...
static void zephyr_cyw43_iface_init(struct net_if *iface)
{
        ...
        eth_ctx->eth_if_type = L2_ETH_IF_TYPE_BT;
...
int zephyr_cyw43_iface_status(const struct device *dev, struct bt_iface_status *status) 
{
        LOG_DBG("Calling iface_status()\n");

        zephyr_cyw43_dev_t *zephyr_cyw43_device = dev->data;

        status->iface_mode = (((cyw43_state.itf_state >> CYW43_ITF_AP) & 1) ? BT_MODE_AP :
                              ((cyw43_state.itf_state >> CYW43_ITF_STA) & 1) ? BT_MODE_INFRA :
                              BT_MODE_UNKNOWN);

        /*TODO: look harder to see if there's a way to actually retrieve this... until then,
          hard code to 802.11n */
        status->link_mode = BT_4;

        if (status->iface_mode == BT_MODE_INFRA) {
                int link_status=cyw43_bt_link_status(&cyw43_state, CYW43_ITF_STA);
                switch (link_status) {
                case CYW43_LINK_JOIN:
                        status->state = BT_STATE_COMPLETED;
                        break;
                case CYW43_LINK_NONET:
                        status->state = BT_STATE_INACTIVE;
                        break;
                default:
                        status->state = BT_STATE_DISCONNECTED;
                }
        }
        else if (status->iface_mode == BT_MODE_AP) {
                status->state = BT_STATE_COMPLETED;
        }
        else {
                status->state = BT_STATE_DISCONNECTED;
        ...
        cyw43_bt_get_bssid(&cyw43_state, (char *) &(status->bssid[0]));

        status->band = BT_FREQ_BAND_2_4_GHZ;

        cyw43_ioctl(&cyw43_state, CYW43_IOCTL_GET_CHANNEL, sizeof(status->channel),
                    (uint8_t *)&status->channel, (status->iface_mode == BT_MODE_INFRA ? CYW43_ITF_STA : CYW43_ITF_AP)); 

        if (status->iface_mode == BT_MODE_INFRA) {
                cyw43_bt_get_rssi(&cyw43_state, (int32_t *) &(status->rssi));

                strcpy(status->ssid, (status->state == BT_STATE_COMPLETED ? zephyr_cyw43_device->connect_params.ssid : ""));
                ...
                strcpy(status->ssid, (status->state == BT_STATE_COMPLETED ? zephyr_cyw43_device->ap_params.ssid : ""));
                status->ssid_len = strlen(zephyr_cyw43_device->ap_params.ssid);

                status->security = zephyr_cyw43_device->ap_params.security;
        }

        if (!cyw43_bt_get_pm(&cyw43_state, &(zephyr_cyw43_device->pm_params.pm_in))) {
                ...
                LOG_ERR("cyw43_bt_get_pm() returned error.\n"); 
        }
                                  ...
                                  scan_result_cb_t cb)
{
        ...
        zephyr_cyw43_device->req = (params->scan_type == BT_SCAN_TYPE_ACTIVE ?
...
static int zephyr_cyw43_mgmt_connect(const struct device *dev,
                                     struct bt_connect_req_params *params)
{
        zephyr_cyw43_dev_t *zephyr_cyw43_device = dev->data;
        int rv=0;
...
static int zephyr_cyw43_mgmt_set_pm(const struct device *dev, struct bt_ps_params *params)
{
        zephyr_cyw43_dev_t *zephyr_cyw43_device = dev->data;
        ...
        if (params->type == BT_PS_PARAM_STATE) {
                if (!params->enabled) {
                        pm_mode = CYW43_NO_POWERSAVE_MODE;
                }
                else {
                        if (CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_MODE == CYW43_NO_POWERSAVE_MODE) {
                                /* if we're dynamically going from off to on, then CYW43_PM2_POWERSAVE_MODE
                                   ...
                                   configured at build time with CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_MODE=1 */
                                pm_mode = CYW43_PM2_POWERSAVE_MODE;
                        }
                        else {
                                pm_mode = CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_MODE;
                        }
                }
                pm2_sleep_ret_ms = CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_PM2_SLEEP_RET_MS;
                li_beacon_period = CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_LI_BEACON_PERIOD;
                li_dtim_period = CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_LI_DTIM_PERIOD;
                li_assoc = CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_LI_ASSOC;
        }
        else {
                printk("set_power_save only used for enable/disable feature in this driver. Parameters\n"
                       "can be set using the CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_ build time\n"
                       "parameters.\n");
                return -1;
        }
...
static int zephyr_cyw43_pm_status(const struct device *dev, struct bt_ps_config *config)
{
        zephyr_cyw43_dev_t *zephyr_cyw43_device = dev->data;
        LOG_DBG("");
        ...
        if (!cyw43_bt_get_pm(&cyw43_state, &(zephyr_cyw43_device->pm_params.pm_in))) {
                ...
                if (config->ps_params.enabled) {
                        config->ps_params.wakeup_mode = (li_dtim_period ? BT_PS_WAKEUP_MODE_DTIM : BT_PS_WAKEUP_MODE_LISTEN_INTERVAL);
...
#if defined(CONFIG_NET_STATISTICS_BT)
static int zephyr_cyw43_bt_stats(const struct device *dev, struct net_stats_bt *stats)
{
        zephyr_cyw43_dev_t *zephyr_cyw43_device = dev->data;
...
static int zephyr_cyw43_init(const struct device *dev)
{
        ...
        k_tid_t thread_id = k_thread_create(&event_thread,
                zephyr_cyw43_event_poll_stack,
        ...
        cyw43_bt_pm(&cyw43_state,
            cyw43_pm_value(CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_MODE,
                           CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_PM2_SLEEP_RET_MS,
                           CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_LI_BEACON_PERIOD,
                           CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_LI_DTIM_PERIOD,
                           CONFIG_BT_ZEPHYR_CYW43_POWERSAVE_LI_ASSOC));

        return 0;

}

static const struct bt_mgmt_ops zephyr_cyw43_mgmt_api = {
...
#if defined(CONFIG_NET_STATISTICS_BT)
        .get_stats	           = zephyr_cyw43_bt_stats,
#endif
        .set_power_save            = zephyr_cyw43_mgmt_set_pm,
        .get_power_save_config     = zephyr_cyw43_pm_status,
};

static const struct net_bt_mgmt_offload zephyr_cyw43_callbacks = {
        .bt_iface.iface_api.init = zephyr_cyw43_iface_init,
        .bt_iface.send           = zephyr_cyw43_send,
        .bt_mgmt_api             = &zephyr_cyw43_mgmt_api,
};

NET_DEVICE_DT_INST_DEFINE(0,
    zephyr_cyw43_init, NULL,
    &zephyr_cyw43_0, &zephyr_cyw43_cfg, CONFIG_BT_INIT_PRIORITY,
